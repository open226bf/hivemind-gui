...
export class ConfigFormComponent {
  ...
  save(): void {
    if (!this.form.name || !this.form.content) {
      this.toast.add({ severity: 'warn', summary: 'Champs requis', detail: 'Nom et contenu sont obligatoires' });
      return;
    }
    ...
    this.api.create({
      ...
      next: () => {
        ...
        this.visible.set(false);
        this.toast.add({ severity: 'success', summary: 'Créée', detail: `Config ${this.form.name}` });
        this.saved.emit();
      },
      error: (err) => {
        this.saving.set(false);
